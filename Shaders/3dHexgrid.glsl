...
float CalculateHeight(vec3 p)
{
    ...
    float overallAmplitude = .5; //Overall amplitude modulation
...
float DF_composition( in vec3 pos )
{
	...
	float height = CalculateHeight(pos - vec3(0.0));
	...
	height = CalculateHeight(pos-vec3(X_REPEAT_DIST*0.5,0, Z_REPEAT_DIST*0.25));
...
{ 
    ...
    return ambientDiffuse(vec3(1),normal);
}
